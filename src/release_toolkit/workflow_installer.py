"""Pure logic for rendering and identifying the release-notify caller workflow.
...
:func:`is_release_notify_workflow`, and write whatever :func:`render_workflow`
...
RELEASE_NOTIFY_USES_REF = f"{RELEASE_NOTIFY_USES_PREFIX}v1"


@dataclass(frozen=True)
class WorkflowConfig:
    """Desired contents of a release-notify caller workflow.
    ...
    def for_monorepo(cls, project_name: str, package_dir: str) -> WorkflowConfig:
        ...
        return cls(
            ...
            workflow_name=f"Release notify ({project_name})",
            file_name=f"release-notify-{project_name}.yml",
        )


class WorkflowInstallStatus(Enum):
    """Outcome of attempting to install a release-notify caller workflow."""
...
def render_workflow(config: WorkflowConfig) -> str:
    """Render a release-notify caller workflow as YAML text.
...
def is_release_notify_workflow(yaml_text: str) -> bool:
    """Return True when ``yaml_text`` references the release-notify reusable workflow.

    Detection is a substring match on :data:`RELEASE_NOTIFY_USES_PREFIX`. The
    prefix is specific enough (full ``owner/repo/.github/workflows/file.yml@``
    path) that no parsing is needed.
    """
    return RELEASE_NOTIFY_USES_PREFIX in yaml_text
